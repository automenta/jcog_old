...
import jcog.nars.reason.language.SetExt;
import jcog.nars.reason.language.SetInt;
import jcog.nars.reason.language.Statement;
import jcog.nars.reason.language.Term;
import jcog.nars.reason.language.Variable;
...
public class StringParser extends Symbols {
    ...
    public void parseTask(String s) {
        ...
        try {
            ...
            NARTruth truth = parseTruth(truthString, punc, memory.getParams().getDefaultJudgmentConfidence());
    ...
    private Term parseCompoundTerm(String s0) throws InvalidInputException {
        ...
        ArrayList<Term> arg = parseArguments(s.substring(firstSeparator + 1)
                + ARGUMENT_SEPARATOR);
        Term t = CompoundTerm.make(memory, op, arg);
        if (t == null) {
