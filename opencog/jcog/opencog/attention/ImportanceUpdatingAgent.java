 ...
 */
public class ImportanceUpdatingAgent extends MindAgent {
	...
	public ImportanceUpdatingAgent() {
		super(ImportanceUpdatingAgent.class.getName());
	...
	@Override public synchronized void run(OCMind mind) {
		this.mind = mind;

                List<MindAgent> agents = getMind().getAgentsActive();
		...
		for (Atom at : h) {
			...
			if (sti > maxSTISeen)	maxSTISeen = sti;
			if (sti < minSTISeen)	minSTISeen = sti;			
		}
	...
	private void updateAtomSTI(List<MindAgent> agents, Atom at) {
	...
	private List<Atom> getAtomsToUpdate() {
		return null;
	}
